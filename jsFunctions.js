function prepareRequest(requestType){
	...
	case 'decode':
    	...
    	var	script = 'decode.php',
			reqNmbr = 1,
    		vars = "jwt="+jwt;

     	//Check for jwt in local storage and if not exist, return apropriate message
		if(jwt==null || jwt == "") {
    ...
    hr.onreadystatechange = function() {
	    if(hr.readyState == 4 && hr.status == 200) {
			...
			if(reqNmbr == 0 || reqNmbr == 1){
				...
				if(reqNmbr == 0){localStorage.setItem("jwt", hr.responseText)}
			...
			if(reqNmbr == 2 ){showImage(hr.responseText)};
	    }
    }
